...
import { SearchIcon, CrossIcon, TrashIcon, PlusCircleIcon } from './Icons';
...
const PayrollView: React.FC<PayrollViewProps> = ({ sellers, sales, layaways, loginHistory, payrollHistory, onSavePayroll, currentUser }) => {
  ...
  // COMISIÓN DESDE LA PRIMERA VENTA
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="bg-white dark:bg-secondary p-6 rounded-xl shadow-lg">
        <h2 className="text-2xl font-bold text-accent mb-6 border-b-2 border-accent/30 pb-2">Gestión de Nómina</h2>
        <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg mb-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
            ...
            <div className="lg:col-span-2">
              <label className="block text-sm font-medium text-gray-500 dark:text-text-dark mb-1">Periodo de Pago</label>
        ...
        {result && (
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2 underline decoration-accent">Liquidación para: {result.sellerName}</h3>
            <p className="text-sm text-gray-500 dark:text-text-dark mb-6">Rango: <span className="font-mono">{result.period}</span></p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-accent p-6 rounded-2xl shadow-xl shadow-accent/20">
                        ...
                        <button onClick={handleRegisterPayment} className="w-full bg-white text-accent font-black py-4 rounded-xl hover:bg-gray-100 transition-all shadow-lg active:scale-95 uppercase tracking-widest">Registrar Pago en Sistema</button>
                    </div>
                </div>
            </div>
            </div>
            ...
            {filteredHistory.length > 0 ? filteredHistory.map(record => {
                ...
                <div key={record.id} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700/60 transition-colors border border-transparent hover:border-accent/20">
                    <div className="flex justify-between items-center cursor-pointer" onClick={() => setExpandedHistoryId(expandedHistoryId === record.id ? null : record.id)}>
                        <div><p className="font-black text-lg text-gray-900 dark:text-white">{record.sellerName}</p><p className="text-sm text-gray-500 dark:text-text-dark">Periodo: {record.period}</p><p className="text-[10px] text-gray-400 uppercase font-bold tracking-tighter">Pagado el {new Date(record.paidAt).toLocaleString()} por {record.paidBy}</p></div>
                        <div className="text-right"><p className="text-2xl font-black text-accent">{formatCOP(record.totalToPay)}</p></div>
                    </div>
                    {expandedHistoryId === record.id && (
                        <div className="mt-4 pt-4 border-t border-gray-300 dark:border-gray-700 text-sm animate-fade-in">
                             <div className="space-y-2 bg-white dark:bg-gray-900/50 p-4 rounded-xl shadow-inner border border-gray-100 dark:border-gray-800">
                                <h4 className="font-black text-center mb-3 text-gray-400 text-xs uppercase tracking-widest">Resumen Liquidación</h4>
