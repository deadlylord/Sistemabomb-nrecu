...
const DashboardView: React.FC<DashboardViewProps> = (props) => {
  ...
  const cashBreakdown = useMemo(() => {
    ...
    const cashIncidents = allIncidents.filter(i => i.storeId === currentStoreId && isWithinRange(i.createdAt) && (i.type === IncidentType.CASH_ADJUSTMENT || i.type === IncidentType.RECAUDO) && (i.paymentMethod ? i.paymentMethod === PaymentMethod.Efectivo : true) && !i.description.includes('Excedente pagado por cambio'));
      ...
      <div className="bg-white dark:bg-secondary p-4 rounded-xl shadow-lg border border-accent/20">
        <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-2 flex-wrap">
                ...
                <button onClick={onOpenVerification} className="px-4 py-1.5 text-sm bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 shadow-md shadow-blue-600/20"><ClipboardListIcon className="w-4 h-4" /><span>Verificar Inventario</span></button>
            </div>
