...
const RecaudoReceiptModal: React.FC<RecaudoReceiptModalProps> = ({ incident, store, onClose }) => {
  ...
  const handlePrint = () => {
    ...
    try {
      // Intentar abrir una ventana emergente para una impresión aislada del comprobante de recaudo.
      ...
      popupWindow.document.write(`
        ...
        <body>
          <div class="text-center mb-2">
            ...
            <div style="font-size: 11px; font-weight: bold; margin-top: 6px;">COMPROBANTE DE RECAUDO</div>
          </div>
          ...
          <table class="w-full mb-2">
            <thead>
              <tr class="border-b-2">
                <th style="text-align: left; font-weight: bold; padding-bottom: 2px;">Concepto</th>
                <th style="text-align: right; font-weight: bold; padding-bottom: 2px; width: 70px;">Valor</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td style="text-align: left; padding: 4px 0; vertical-align: top;">
                  ${incident.type}<br/>
                  <span style="font-size: 9px; color: #333;">${incident.description}</span>
                </td>
                <td style="text-align: right; padding: 4px 0; vertical-align: top;">${formatCOP(incident.adjustmentAmount || 0)}</td>
              </tr>
            </tbody>
          </table>
          <div class="border-t-2" style="padding-top: 4px; text-align: right; font-size: 11px;">
            <p style="margin: 3px 0 0 0; font-size: 12px;"><strong>Total Recibido: ${formatCOP(incident.adjustmentAmount || 0)}</strong></p>
          </div>
      ...
      // Fallback native print on main window
      window.print();
    }
  };
