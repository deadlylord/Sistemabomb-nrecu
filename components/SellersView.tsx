...
interface SellersViewProps {
  ...
  roles: Role[];
  stores: Store[];
  onAddSeller: (name: string, password: string, roleId: string, storeId: string) => void;
  onUpdateSeller: (id: string, newName: string, newPassword: string, newRoleId: string, newStoreId: string) => void;
  onDeleteSeller: (id: string) => void;
  onToggleSellerStatus: (id: string) => void;
}

const SellersView: React.FC<SellersViewProps> = ({ sellers, roles, stores, onAddSeller, onUpdateSeller, onDeleteSeller, onToggleSellerStatus }) => {
  ...
  const handleSave = (name: string, password: string, roleId: string, storeId: string) => {
    if (editingSeller) {
      onUpdateSeller(editingSeller.id, name, password, roleId, storeId);
    } else {
      onAddSeller(name, password, roleId, storeId);
    }
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filteredSellers.map((seller) => (
                  <tr key={seller.id} className={`transition-colors ${seller.isDisabled ? 'bg-red-50 dark:bg-red-900/20 opacity-60' : 'hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                    <td className="p-3 font-bold">{seller.name}</td>
                    <td className="p-3">
